...
def view_plot(plot, viewer='feh'):
    child = subprocess.Popen([viewer, '-'], stdin=subprocess.PIPE)
    plot.savefig(child.stdin, format='png')
    return child
...
print("\n\nStep: {}\nLoss: {}\n\n".format (global_step.numpy(), loss(model, features, labels).numpy()))
...
for epoch in range(epochs):
    loss_average = eager.metrics.Mean()
    accuracy = eager.metrics.Accuracy()
    ...
    for x, y in train_dataset:
        ...
        max = tf.argmax(model(x), axis=1, output_type=tf.int32)
        accuracy(max, y)
        
    train_loss_results.append(loss_average.result())
    train_accuracy_results.append(accuracy.result())

figure, axes = plot.subplots(2, sharex=True, figsize=(12, 8))
...
test_accuracy = eager.metrics.Accuracy()
